...
| `claude-config/settings.json` → `.claude/settings.json` | `permissions.deny` on every team-lead file (single writer per file); hooks: `SessionStart` (refresh + inject `knowledge/hot.md`, stale check, context census), `Stop` (`brain-session-end.py`: daily-log stub + `knowledge/index.md`), `PreToolUse(Bash)` (`refuse_sweeping_commands.py` + a code-graph hint before greps) |
